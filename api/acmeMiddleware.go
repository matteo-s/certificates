...
func (h *caHandler) addNonce(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		nonce, err := h.Authority.NewNonce()
		if err != nil {
			WriteError(w, err)
			return
		}
...
func (h *caHandler) addDirectory(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Link", link(h.Authority.GetLink(acme.DirectoryLink, true), "index"))
		next(w, r)
		return
	}
}
...
func (h *caHandler) verifyContentType(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		expected := "application/jose+json"
		contentType := r.Header.Get("Content-Type")
		if contentType != expected {
			WriteError(w, acme.MalformedErr(errors.Errorf(
...
func (h *caHandler) parseJWS(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		body, err := ioutil.ReadAll(r.Body)
		if err != nil {
			WriteError(w, acme.ServerInternalErr(errors.Wrap(err, "failed to read request body")))
			return
		}
		jws, err := jose.ParseJWS(string(body))
		if err != nil {
			WriteError(w, acme.MalformedErr(errors.Wrap(err, "failed to parse JWS from request body")))
			return
		}
...
func (h *caHandler) validateJWS(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		jws, ok := jwsFromContext(r)
		if !ok || jws == nil {
			WriteError(w, acme.ServerInternalErr(errors.Errorf("jws not in request context")))
			return
		}
		if len(jws.Signatures) == 0 {
			WriteError(w, acme.MalformedErr(errors.Errorf("request body does not contain a signature")))
			return
		}
		if len(jws.Signatures) > 1 {
			WriteError(w, acme.MalformedErr(errors.Errorf("request body contains more than one signature")))
			return
		}
		...
		if len(uh.KeyID) > 0 ||
			...
			WriteError(w, acme.MalformedErr(errors.Errorf("unprotected header must not be used")))
			return
		}
		hdr := sig.Protected
		if hdr.Algorithm == "none" {
			WriteError(w, acme.MalformedErr(errors.Errorf("algorithm cannot be none")))
			return
		}

		// Check the validity/freshness of the Nonce.
		if err := h.Authority.UseNonce(hdr.Nonce); err != nil {
			WriteError(w, err)
			return
		}
		...
		if !ok || len(jwsURL) == 0 {
			WriteError(w, acme.MalformedErr(errors.Errorf("JWS missing url protected header")))
			return
		}
		reqURL := &url.URL{Scheme: "https", Host: r.Host, Path: r.URL.Path}
		if jwsURL != reqURL.String() {
			WriteError(w, acme.MalformedErr(errors.Errorf("url header in JWS (%s) does not match request url (%s)", jwsURL, reqURL)))
			return
		}

		if hdr.JSONWebKey != nil && len(hdr.KeyID) > 0 {
			WriteError(w, acme.MalformedErr(errors.Errorf("jwk and kid are mutually exclusive")))
			return
		}
		if hdr.JSONWebKey == nil && len(hdr.KeyID) == 0 {
			WriteError(w, acme.MalformedErr(errors.Errorf("either jwk or kid must be defined in jws protected header")))
			return
		}
		next(w, r)
		return
...
// middleware.
func (h *caHandler) extractJWK(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		jws, ok := jwsFromContext(r)
		if !ok || jws == nil {
			WriteError(w, acme.ServerInternalErr(errors.Errorf("jws expected in request context")))
			return
		}
		jwk := jws.Signatures[0].Protected.JSONWebKey
		if jwk == nil {
			WriteError(w, acme.MalformedErr(errors.Errorf("expected jwk in protected header")))
			return
		}
		ctx = context.WithValue(ctx, jwkContextKey, jwk)

		acc, err := h.Authority.GetAccountByKeyID(jwk.KeyID)
		switch {
		...
		default:
			if acc.IsValid() {
				WriteError(w, acme.UnauthorizedErr(errors.New("acme account is not active")))
				return
			}
...
func (h *caHandler) lookupJWK(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		jws, ok := jwsFromContext(r)
		if !ok || jws == nil {
		...
		kidPrefix := h.Authority.GetLink(acme.AccountLink, true, "")
		kid := jws.Signatures[0].Protected.KeyID
		if !strings.HasPrefix(kid, kidPrefix) {
			WriteError(w, acme.MalformedErr(errors.Errorf("kid does not have "+
				"required prefix; expected %s, but got %s", kidPrefix, kid)))
			return
		}

		accID := strings.TrimPrefix(kid, kidPrefix)
		acc, err := h.Authority.GetAccount(accID)
		switch {
		case nosql.IsErrNotFound(err):
...
// Make sure to parse and validate the JWS before running this middleware.
func (h *caHandler) verifyAndExtractJWSPayload(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		jws, ok := jwsFromContext(r)
		if !ok || jws == nil {
			WriteError(w, acme.ServerInternalErr(errors.Errorf("jws expected in request context")))
		...
		if err != nil {
			WriteError(w, acme.MalformedErr(errors.Errorf("failed to verify jws")))
			return
		}
...
func (h *caHandler) isPostAsGet(next nextHTTP) nextHTTP {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !payload.isPostAsGet {
			WriteError(w, acme.MalformedErr(errors.Errorf("expected POST-as-GET")))
			return
		}
		next(w, r)
		return

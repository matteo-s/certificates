...
func TestOrderToACME(t *testing.T) {
	...
	for name, run := range tests {
		tc := run(t)
		t.Run(name, func(t *testing.T) {
			acmeOrder, err := tc.o.toACME(nil, dir)
			if err != nil {
				if assert.NotNil(t, tc.err) {
					ae, ok := err.(*Error)
					assert.True(t, ok)
				...
				if assert.Nil(t, tc.err) {
					...
					assert.Equals(t, acmeOrder.Finalize, fmt.Sprintf("https://ca.smallstep.com/acme/order/%s/finalize", tc.o.ID))
					if tc.o.Certificate != "" {
						assert.Equals(t, acmeOrder.Certificate, fmt.Sprintf("https://ca.smallstep.com/acme/certificate/%s", tc.o.Certificate))
					}
...
func TestOrderFinalize(t *testing.T) {
	...
	tests := map[string]func(t *testing.T) test{
		...
		"fail/ready/csr-names-match-error-2": func(t *testing.T) test {
			...
			csr := &x509.CertificateRequest{
				...
				DNSNames: []string{"bar", "step.example.com"},
			}
			return test{
				o:   o,
				csr: csr,
	...
	for name, run := range tests {
		t.Run(name, func(t *testing.T) {
			tc := run(t)
			o, err := tc.o.finalize(tc.db, tc.csr, tc.sa)
			if err != nil {
				if assert.NotNil(t, tc.err) {
					ae, ok := err.(*Error)
					assert.True(t, ok)
